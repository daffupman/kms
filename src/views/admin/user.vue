<template>
  <a-layout>
    <a-layout-content :style="{ background: '#fff', padding: '24px', margin: 0, minHeight: '280px' }">
      ...
      <a-table :columns="columns" :row-key="record => record.id" :data-source="users" :pagination="pagination" :loading="loading" @change="handleTableChange">
...
export default defineComponent({
  ...
  setup() {
    ...
    const passwordReg = /^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6,30}$/;
    ...
    const handleModalOk = () => {
      const originPassword = user.value.password;
      // 密码校验
      if (!checkPassword(user.value.password)) {
        message.error("【密码】至少包含 数字和英文，长度6-30");
        return;
      }
      ...
      axios.put("/notes/user", user.value).then((resp) => {
        ...
        if (response.ok) {
          ...
          user.value.password = originPassword;
          message.error(response.msg);
        }
      });
    };
    ...
    const handleDelete = (id: number) => {
      axios.delete("/notes/user/" + id).then((resp) => {
        const response = resp.data; // data = commonResp
        if (response.ok) {
          // 重新加载列表
          handleQuery({
    ...
    const handleResetModalOk = () => {
      ...
      user.value.password = hexMd5(user.value.password + KEY);
      axios.patch("/notes/user/resetting", {
        id: user.value.id,
